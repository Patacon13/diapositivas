...
Este archivo define las restricciones académicas, pedagógicas y de estilo específicas para las diapositivas de **AEDD** (UTN Santa Fe).
...
## 2. Restricciones Académicas Estrictas
...
## 3. Componentes Visuales Habituales
- **Badges de incisos**: `.inciso-badge` (círculo dorado con letra para problemas de examen/práctica).
- **Diagramas de arreglos**: Usar flexbox puro `.array-diagram` y `.array-cell` en lugar de capturas PNG.
- **Ventana de terminal**: `.terminal-window` con botones de control estilo macOS / Linux.
